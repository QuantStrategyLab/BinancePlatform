...
class WatchdogWorkflowTests(unittest.TestCase):
    ...
    def test_runtime_workflow_uses_cached_uv_environment(self) -> None:
        ...
        self.assertIn('"$VENV_PATH/bin/python" -m ensurepip', text)
        self.assertNotIn('"$VENV_PATH/bin/python" -m ensurepip --upgrade', text)
        self.assertIn('"$VENV_PATH/bin/python" -m pip install pip uv', text)
        self.assertNotIn('"$VENV_PATH/bin/python" -m pip install --upgrade pip uv', text)
