...
class WatchdogWorkflowTests(unittest.TestCase):
    ...
    def test_runtime_workflow_uses_cached_uv_environment(self) -> None:
        ...
        self.assertIn('UV_TOOL_VENV="${CACHE_ROOT}/uv-tool"', text)
        self.assertIn('"$UV_TOOL_VENV/bin/python" -m ensurepip', text)
        self.assertNotIn('"$UV_TOOL_VENV/bin/python" -m ensurepip --upgrade', text)
        self.assertIn('"$UV_TOOL_VENV/bin/python" -m pip install pip uv', text)
        self.assertNotIn('"$UV_TOOL_VENV/bin/python" -m pip install --upgrade pip uv', text)
        self.assertNotIn('"$VENV_PATH/bin/python" -m pip install pip uv', text)
        self.assertIn('export UV_PROJECT_ENVIRONMENT="$VENV_PATH"', text)
        self.assertIn('UV_BIN="$UV_TOOL_VENV/bin/uv"', text)
        self.assertNotIn('UV_BIN="$VENV_PATH/bin/uv"', text)
