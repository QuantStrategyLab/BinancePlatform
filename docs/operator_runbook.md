...
- The in-repo installer template is `.github/workflows/install-dispatch-guard.yml` (`~/binance-quant/ops/dispatch-runtime.sh` + `~/.config/binance-platform/dispatch.env` with required `REF`). The live VPS `REF`/cron owner is outside this repository; do not infer or rewrite it from source alone.
- Dispatch `REF` resolves to the workflow YAML revision GitHub loads (`github.sha`). That trigger SHA is verified against `BINANCE_RUNTIME_WORKFLOW_SHA` and is **not** the application execution identity (`BINANCE_RUNTIME_RELEASE_SHA`). If production still dispatches floating `main`, any later main tip that differs from the workflow pin fails closed even when the app pin is unchanged; keep `REF` aligned with the approved workflow revision under separate authorization, and do not chase tip by auto-approving arbitrary main.
...
- A live startup failure while reading the committed recovery-control document is reported as `recovery_control_read_failed`. This code covers only the read boundary; payload validation and authorization failures retain their existing reason codes. It does not retry the read or change recovery state.
