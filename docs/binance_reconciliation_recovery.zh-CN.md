...
实际运行加载策略池时，已启用 prospective Earn 会计的账本以
`earn_accrual_checkpoint.assets` 固定批准范围，并要求 `last_balance_snapshot` 保持完全相同的
资产 key；较早的 rebase 账本继续从现有 snapshot 取得范围。范围外且无持仓的上游候选只从
策略候选中剔除，来源 payload 和版本保持原样；范围外真实持仓、错误交易对或专用资产混入趋势池
仍会阻断。未被本期池选中的批准资产继续参加余额读取、估值和快照，但不能成为买入候选。
BTC、BNB、USDT 保留固定角色，不能混入趋势部分重复计值。本次不新增资产清单服务。
